...
def model_selector(model, filename):
    if model == "diep":
        folder_tag = model + "_" + filename
        model_src = model
    elif model == "mym3gnet":
        model_src = "/home/abshe/matgl/src/my_models/m3gnet_pes"
        folder_tag = "mym3gnet_" + filename
    elif model == "m3gnet":
        folder_tag = "m3gnet_" + filename
        model_src = model
    return model_src, folder_tag


log = open("test_ga/" + model + "_log", "w")
g = glob.glob("test_ga/sample_elemental/*.cif")
g_done = glob.glob("og_lab/ga_*")
g_done = [x.replace("og_lab/ga_" + model + "_", "") for x in g_done]
for gg in g:
    ...
    log.flush()
    if len(s) > 12 or set(og().atomic_numbers) == {1} or len(set(og().atomic_numbers).intersection(range(57, 104))) > 0:
