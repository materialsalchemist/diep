...
sample_elemental = glob.glob("test_ga/sample_elemental/*.cif")
...
for a in sample_elemental:
    sample_elemental_structures[a] = OgStructure(file_name="test_ga/sample_elemental/" + a + ".cif")

f = open("test_ga/results_" + model + ".csv",'w')
f.write("material,ga,rms\n")
f_summary = open("test_ga/results_summary_" + model + ".csv",'w')
f_summary.write("material,ga,rms\n")
for k in sample_elemental_structures.keys():
    ga_files = glob.glob("og_lab/ga_" + model + "_" + k + "/relaxed/*.cif")
    if ga_files:
