...
def get_pair_vector_from_graph(g: dgl.DGLGraph):
    ...
    bond_vec[:, :] = g.ndata["pos"][g.edges()[1][:].long(), :] + torch.squeeze(
        torch.matmul(
    ...
    return (
        g.ndata["pos"][g.edges()[0][:].long(), :].double(),
        bond_vec,
...
def triangle_transform_cpu(R1, R2, R3, Z1, Z2, Z3):
    ...
    ZZ12a = Z_idx.gather(
        1, torch.stack([order_indices[:, 2] * 2, order_indices[:, 2] * 2 + 1], dim=1)
    )  # batch x 2
    ZZ13b = Z_idx.gather(
        1, torch.stack([order_indices[:, 0] * 2, order_indices[:, 0] * 2 + 1], dim=1)
    )

    ZZ1arr = torch.where(ZZ12a[:, 0] == ZZ13b[:, 0], ZZ12a[:, 0], ZZ12a[:, 1])
    ones_ZZ1 = torch.ones(num_triplets).to(device=torch.device(device))
    ZZ1arr = torch.where(
        (ZZ12a[:, 0] != ZZ13b[:, 0]) & (ZZ12a[:, 1] != ZZ13b[:, 1]), ones_ZZ1, ZZ1arr
    )
    ZZ1 = torch.gather(
        Z_array, 1, torch.unsqueeze(ZZ1arr.type(torch.int64), dim=1)
    ).squeeze()
    ZZ2idx = torch.where(ZZ12a[:, 0] == ZZ1arr, ZZ12a[:, 1], ZZ12a[:, 0])
    ZZ2 = torch.gather(
        Z_array, 1, torch.unsqueeze(ZZ2idx.type(torch.int64), dim=1)
    ).squeeze()
    ZZ3idx = torch.where(ZZ13b[:, 0] == ZZ1arr, ZZ13b[:, 1], ZZ13b[:, 0])
    ZZ3 = torch.gather(
        Z_array, 1, torch.unsqueeze(ZZ3idx.type(torch.int64), dim=1)
    ).squeeze()
    ...
    try:
        ...
        r3_y = f(r3_y).astype(
            np.float32
        )  # If d1 = d2 + d3, then just set r3_y to zero, it's a straightline
    ...
    fR1 = fR1.to(device=torch.device(device))
    fR2 = fR2.to(device=torch.device(device))
    fR3 = fR3.to(device=torch.device(device))
...
def integration_cpu(tR1, tR2, tR3, tZ1, tZ2, tZ3, do_sum=True):
    ...
    n = torch.exp(-torch.norm(tR1_batch - grid_batch, dim=2) ** 2) + torch.exp(
        -torch.norm(tR2_batch - grid_batch, dim=2) ** 2
    )
    ...
    zeros.to(device=torch.device(device))
    ones = torch.ones(121, batchlen)
    ones.to(device=torch.device(device))

    d_batch = (
        n * tZ1_batch / torch.sqrt(d1_batch**2 + 1)
        + n * tZ2_batch / torch.sqrt(d2_batch * 2 + 1)
        + n * tZ3_batch / torch.sqrt(d3_batch * 2 + 1)
    )
...
def calculate_dsum_cpu(
    grid, tR1, tR2, tR3, num_points_square, tZ1, tZ2, tZ3, do_sum=True, max_n=1
):
    batchlen = len(tR1)
    grid_batch = torch.unsqueeze(grid, dim=1)
    grid_batch = grid_batch.repeat(1, len(tR1), 1)
    ...
    for i in range(1, max_n + 1):
        ...
        d_batch = (
            tZ1_batch / torch.sqrt(d1_batch**2 + i)
            + tZ2_batch / torch.sqrt(d2_batch * 2 + i)
            + tZ3_batch / torch.sqrt(d3_batch * 2 + i)
        )
        if do_sum:
            d_sums = torch.sum(d_batch, dim=0)
            d_sums = torch.nn.functional.normalize(d_sums, dim=0)
...
def integration_multiple_meshes_cpu(max_n, tR1, tR2, tR3, tZ1, tZ2, tZ3, do_sum=True):
    ...
    grid = [
        [(x, y) for x in range(-2 * grid_size, 2 * grid_size + 2, 2)]
...
def three_body_dft_integral_multiple_meshes(
    graph, triple_bond_indices, max_n, do_sum=True
):
    R1, R2, R3, Z1, Z2, Z3 = extract_vectors_from_graph(graph, triple_bond_indices)
    tR1, tR2, tR3, tZ1, tZ2, tZ3 = triangle_transform(R1, R2, R3, Z1, Z2, Z3)
